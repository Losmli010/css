  ...
  <div>
    ...
    <p>
      <a href="https://github.com/xieranmaya/blog/issues/3" target="_blank">
        剖析Promise内部结构，一步一步实现一个完整的、能通过所有Test
        case的Promise类
      </a>
    </p>
    <ul>
    ...
    <ul>
      ...
      <li>
        ...
        <button class="btn" @click="handlePromiseError">
          Promise 异常处理
        </button>
      </li>
      <li>
        永远不要在 tasks 队列中抛出异常，因为 tasks
        队列脱离了运行上下文环境，异常无法被当前作用域捕获。
        <button class="btn" @click="cannotHandleError">
          Promise 无法捕获的异常
        </button>
      </li>
      <li>
        只要注意在 tasks 级别回调中使用 reject，就没有抓不住的异常。
        <button class="btn" @click="canHandleError">
          Promise 捕获第三方异常
        </button>
      </li>
    </ul>
    ...
    <ul>
      <li>
        使用 try catch 捕获异常。
        <button class="btn" @click="catchError">Async Await 捕获异常</button>
      </li>
      <li>
        不仅是 reject，抛出的异常也会被作为拒绝状态被 Promise 捕获。
        <button class="btn" @click="cannotCatchError">
          Async Await 无法捕获的异常
        </button>
      </li>
    </ul>
